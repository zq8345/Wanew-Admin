...
// ---------- 仪表盘接口：已删除（前端仪表盘一并删掉，无调用方）----------
// 🔴 徽标改为**前端从已加载的 products 现算** —— 与列表筛选同一份数据、同一条谓词，
//    所以"徽标数字 == 筛选条数"是构造上成立的，而不是两处各算一遍碰巧相等。
// 取回：git show 93d1ca37:src/index.ts（含 todo / recent / lastPublish 三段计算）
//    ⚠️ 恢复之前先问那个问题：那些数字点了能干活吗？不能就别恢复。
