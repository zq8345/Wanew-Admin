...
//    按旧模型，保存产品（33 文件）会被算成需要 38 次而被自己的闸拒掉，实际只花个位数。
//    **一个模型过时了的安全装置，就是停机器。**
// 所以这里机器核对，不靠记性：数 commitFiles 里的 `await gh(` 调用点，并确认循环里没有网络请求。
//
// ⚠️ **这道闸的适用条件要说清，别以为它能自适应任何改动**：
//    它假定 **真实开销 == `await gh(` 调用点数**。2026-07-28 官网加了一次
//    `GET /git/trees/{sha}?recursive=1`（5→6），调用点也 +1，所以它自己跟上了 ——
//    🔴 **但如果官网把两次请求合进一个辅助函数**，调用点数不变而开销变了，
//    这道闸会**静默地继续说"对"**。那种改动只能靠 re-vendor 时读 diff 发现。
//    ⇒ 它挡的是"数字忘了改"，不是"实现换了形状"。
// ⚠️ 下面所有对外输出的数字一律用 `base`，**不写字面量** —— 这个文件本身就是
//    "写死的旧数会被当实测引用"的反面教材来源，不该自己再犯一次。
{
  ...
  // ⚠️ 两条路径必须都被模型认得：只按固定值算会在退路上撞限，只按固定值+N 算会误拒正常保存
  ...
  console.log(`✅ 成本模型 PASS：固定 ${base} 次${blobPath ? ` + 超限退路(按 ${base}+文件数 计)` : ""}${vLimit ? ` · 阈值两边一致(${vLimit.trim()})` : ""}。`);
}
